...
I'm gonna work on making some box plots. 

So, in order to do this, I would be using data from each variant-transcript pair across the 0, 1 and 2 genotypes. We also have counts (n), nrows, and individual. So I think we will be collapsing this data on variant/transcript pair and plotting aggregated counts. Another thing is that I will need to devise a way to narrow down which variant/transcript pairs get boxplotted. I think I can do this by degree to which the counts across all individuals change across genotypes (delta).

To-do list to keep myself organized:
- Narrow var/trans to those which change the most (maybe top 20 genes? (out of a bajillion))
- Collapse across all tissues according to var/trans pair
- (((plot counts of each genotype) for each individual) for each narrowed-down var/trans pair)

#### 04/20/2020

I talked to Brad Sherman from DAVID and he told me that RDAVID does not support gene symbols because of ambiguity and he also told me the reason the Ensembl IDs weren't working was because they have version numbers at the end of them, which he removed for me. The version with no version numbers is in `results/`. I should be able to use that to find the functional categories.

#### 04/21/2020

I made a rudiementary way of finding "enrichment" for "functional annotation categories" in `geneGOTerms.R` but the problem is, I have no idea if this is enough or not (my money is not). But I have something which is important and good.

Going back to last Thursday, I already wrote a script that would find the difference between number of counts between the different genotypes. I just have to reuse that but for much larger files. I guess I also have to horizontalize them. Not sure if I should collapse across all tissues, though, since they're not really comparable like that AFAIK. I have a big brain for statistics.

`merge_tables.R` makes the `*_NL_isos.txt` files, so I guess I have to make them horizontal and then find the difference again.

#### 04/22/2020

Let's go, baby. It's Wednesday.

#### 04/28/2020

I met with Steph yesterday. Her notes were the following:

> 1. maybe find a way to find the % of genes that are involved in immune function
2. find the difference between homo human and hetero HN
3. use the the horizontal files you already have
4. ~400 unique transcripts, see if the number of variants for the transcript I'm looking at matches the number of variants in the loosenedRestrictions file
 
 I should just generate a new batch of NL isoform files just to be safe. Then, I can horizontalize them like last time and then find the difference between homozygous human vs heterozygous counts for each sQTL + transcript combo across all tissues.
 
 In other news, Rajiv asked me to test out our dev node's new `rstudio-server-start` function, which allows us to use our dev node to develop code in RStudio on our computers, which is a godsend. His instructions are below:
 
>1. Login to one of the dev nodes as usual. rmccoy22-dev or rmccoy22-dev02
2. Load the R module using module load R.
3. Fake a SLURM job ID by entering export SLURM_JOBID=001.
4. Next run the command rstudio-server-start and follow the onscreen instructions. These involve copying and pasting an ssh command into a separate terminal, then opening an http://localhost:<number_here>/ link in your web browser.
5. Sign into the Rstudio cloud session in your browser using your usual MARCC username (<jhed>@jhu.edu) and password.
6. Use pbmclapply and set the  mc.cores argument to parallelize your apply functions and speed up your analysis, for example.

The problem is, however, that I get no response to step #4 when I `ssh` in a new terminal, and the website doesn't work. I emailed MARCC help about this. Let's see what they say. If they don't respond, I might try again or reach out to them again.

I was able to generate a list of genes in the "top DAVID cluster," all of which were tagged as having some sort of immune system function. They're in `results/NL_Iso_TopGenes_List.txt`. We can't really do any sort of "in-depth" analysis on this list other than to take the names and point at them and say we found a few guys that have been linked to some sort of immune system process, like I said before. We can use this for the paper, is all I'm saying.

Next, I need to figure out the deal with the boxplots. I'll try to do that after fixing RStudio server to make the process a bit easier.

#### 04/29/2020
Matthew from MARCC-help got back to me about my email. Here are his recommendations:

```
0. ssh rmccoy22-dev
1. screen -ls    # to check any preexisting rstudio screens. Attach and kill as needed. (pkill screen)
2. ml restore    # clean up module environment
3. ml -emacs    # the emacs module causes problems with rstudio
4. ml R/3.6.1    # or any R you would like to use
5. export SLURM_JOBID=001    # fake a slurm job ID in case of complaints
6. rstudio_server_start    # wait for instructions. After receiving them, you can Ctrl-c to get out
7. screen -ls     # to confirm that the rstudio screen session is working

On your local machine now:
1. Open a terminal and use the instructions where #### is the port number from the instructions:
     ssh -N -L ####:rmccoy22-dev:#### username@school.edu@login.marcc.jhu.edu
2. After typing your 2FA and password, put the following in a browser:
     http://localhost:####/ 
  2a.You must use your full email address, with @jhu.edu
  2b. If you are experiencing issues, try another browser. I had to use Microsoft Edge.

Clean up your screen at the end:
1. screen -ls
2. screen -a
3. Ctrl-a, then hit 'k' for kill, 'y' to confirm.
```

Well, I almost got it to work, and then I broke it by entering my log in info into RStudio server without the `@jhu.edu` at the end. So I let Matt know and now he's working on it. Hopefully it'll be all fixed up soon.

#### 05/01/2020

R studio server works now.

#### 05/04/2020

RStudio Server's packages don't work. I sent an email to MARCC help desk. Let's see what they say.

Apart from that, I submitted a job to find the differences in counts between heterozygous human/NL and homozygous human transcript counts for each variant/transcript combo. It's currently in queue on MARCC, 

#### 05/07/2020

I met w Steph, she said I gotta make a box plot of this sQTL:

```
transcript: chr2_3769142_3769310
sQTL: chr2_3769143_C_T_b38
tissue: testis
```

#### 05/11/2020

I found the differences I was looking for as mentioned two entries ago. Now I need to figure out how to make box plots of some of the highest difference sQTL/transcripts.

#### 05/15/2020

After a fiasco this past Tuesday involving Chinese hackers compromosing MARCC and me getting kicked off and having to reset my password 5 times, I'm finally back. I've made a chart of sorts. Best to assume it's inadequate.

MARCC is being insanely slow at processing jobs. 

#### 05/18/2020

We need not the averages but a box plot which shows each individual as a point on the graph. So we need to go back and use a different format of file.

#### 05/19/2020

IBDMix apparently added a feature that reports introgressed SNPs so Rajiv went ahead a called Neandertal-introgressed SNPs, and was able to find the sQTL for ADAMTSL3, which is good for the scientific integrity of this paper.

```
[rmccoy22@jhu.edu@rmccoy22-dev02 ibdmix_summary]$ cat gtex_ibdmix_chr15_50000_lod4.txt | tr ',' '\t' | grep -w 84037709 | wc -l
90
```

I saved Rajiv's code in a file called `IBDMix.sh`. He said:
> I'll work on reformatting this as a simple list of introgressed SNPs--i.e., the generate_gt file, but restricted to these positive LOD-score SNPs that define significant introgressed haplotypes. 

#### 05/20/2020

I was offered a position in another lab and my current position ends next Friday so this log is done and I will be collecting all of the relevant information 
